...
pair white_black_areas(size_t n, const unsigned rows[n], const unsigned cols[n]);

int main() {
    const unsigned rows[5] = {3, 1, 2, 7, 1};
    const unsigned cols[5] = {1, 8, 4, 5, 2};
    size_t n = sizeof(rows) / sizeof(unsigned);

    white_black_areas(n, rows, cols);
}

pair white_black_areas(size_t n, const unsigned rows[n], const unsigned cols[n]) {
    pair board;
    if (n == 1) {
        board.white = rows[0] * cols[0];
        board.black = 0;
