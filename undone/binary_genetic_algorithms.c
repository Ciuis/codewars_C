...
int main() {
    ...
    size_t length = 22;
    char* bga = generate(length);

    printf("Base chromosome: \n");
    for (int i = 0; i < length; ++i) {
        printf("%d ", bga[i]);
    }

    printf("\n\nMutable chromosome:\n");
    char* newChromosome = mutate(length, bga, 0.5);

    for (int i = 0; i < length; ++i) {
        printf("%c ", newChromosome[i]);
    }

}

char* generate (size_t length) {
    char* chromosome = malloc(length * sizeof(char));
    for (int i = 0; i < length; ++i) {
        chromosome[i] = rand() % 2;
    }
...
void select (int size, char* population[size], double fitnesses[size]) {
    ...
    for (int i = 0; i < size; ++i) {
        samps[i] = pSamp + (fitnesses[i] / sum);
        pSamp = samps[i];
    }

    while (rand1 > samps[cnt]) {
        cnt++;
    }

    char* sChromosomes[2];
    sChromosomes[0] = population[cnt];
    cnt = 0;

    while (rand2 > samps[size]) {
        cnt++;
    }

    sChromosomes[1] = population[cnt];

}

char* mutate (size_t n, char* s, double p) {
    char* newChromosome = malloc(n * sizeof(char));


    for (size_t i = 0; i < n; ++i) {
        double sample = rand() % 2;
        char c = s[i] + '0';
        //printf("%c ", c);
...
void crossover (size_t n, char* s1, char* s2) {
    char** newChromosomes[2];
    double var = floor(rand() * n);
